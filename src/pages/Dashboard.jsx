...
import { batches, enrollmentTrends, students } from "../data/mockData";
  ...
  const stats = [
    {
      ...
      glow: "from-primary to-amber-500",
    },
    {
  ...
  return (
    <div className="space-y-8 sm:space-y-10">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm sm:text-base text-text-muted font-medium opacity-80">
        ...
        {stats.map((stat, i) => (
          ...
          >
            <Card className="relative overflow-hidden group">
              <div
                className={`absolute -top-10 -right-10 w-28 h-28 rounded-full bg-gradient-to-br ${stat.glow} opacity-5 group-hover:opacity-15 blur-2xl transition-opacity duration-500`}
              />
              <div className="flex items-center justify-between relative z-10">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-2xl sm:text-3xl font-black text-text-primary mb-3 tabular-nums">
                    {stat.value}
                  </p>
                  <div className="flex items-center gap-2">
                    <div
                      className={`flex items-center gap-1 px-1.5 py-0.5 rounded-lg ${stat.trend === "up" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
                    >
                      {stat.trend === "up" ? (
                        <TrendingUp size={12} />
                      ) : (
                ...
                <div
                  className={`w-14 h-14 rounded-2xl ${stat.bg} flex items-center justify-center shrink-0 shadow-inner group-hover:scale-110 transition-transform duration-300`}
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
            <div>
              ...
              <p className="text-sm text-text-muted mt-1 font-medium">
                Monthly student enrollments over the past year
              </p>
            </div>
          ...
          <div className="flex items-end gap-2 sm:gap-4 h-48 sm:h-64 relative">
            ...
            {enrollmentTrends.map((d, i) => (
              ...
              >
                ...
                <span className="text-[10px] sm:text-xs font-bold text-text-muted uppercase tracking-tighter sm:tracking-normal">
                  {d.month}
                </span>
              </div>
            ))}
            ...
            <div className="space-y-6">
              {sortedBatches.slice(0, 5).map((batch, i) => {
                const pct = Math.round(
                  (batch.studentCount / batch.maxLimit) * 100,
                );
                return (
                  <div key={batch.id} className="flex items-center gap-5 group">
                    <span className="w-8 h-8 rounded-xl bg-surface-alt border border-border/50 flex items-center justify-center text-xs font-black text-text-muted shrink-0 group-hover:text-primary transition-colors">
                      {i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-sm font-bold text-text-primary truncate group-hover:translate-x-1 transition-transform">
                          {batch.name}
                        </p>
                        <span className="text-xs text-text-muted font-bold ml-2 shrink-0 tabular-nums">
                          {batch.studentCount} / {batch.maxLimit}
                        </span>
                      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-text-muted font-semibold mt-0.5">
                        {batch.studentCount} students enrolled · {pct}% capacity
                      </p>
                    </div>
                    <div
                      className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider shrink-0 ${batch.status === "open" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}
                    >
