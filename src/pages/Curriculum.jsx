...
const transformMockData = (items, defaultType = "subject") => {
  ...
  return items.map((item) => {
    const children = [];
    if (item.modules)
      children.push(...transformMockData(item.modules, "module"));
    if (item.chapters)
      children.push(...transformMockData(item.chapters, "chapter"));
...
export default function Curriculum() {
  const [subjects, setSubjects] = useState(() =>
    transformMockData(initialSubjects, "subject"),
  );
  ...
  const handleDragOver = (event) => {
    ...
    setSubjects((prev) => {
      ...
      const traverse = (items, path) => {
        ...
        items.forEach((item) => {
          if (item.children)
            traverse(item.children, [...path, { id: item.id }]);
        });
      };
      ...
      if (!overContainerPath) {
        ...
        const findContainerById = (items, path) => {
          ...
          for (const item of items) {
            if (item.id === overId)
              return { path: [...path, { id: item.id }], items: item.children };
            if (item.children) {
              const res = findContainerById(item.children, [
                ...path,
                { id: item.id },
              ]);
              if (res) return res;
      ...
      const isSameContainer =
        JSON.stringify(activeContainerPath) ===
        JSON.stringify(overContainerPath);
      ...
      overList.splice(
        overIndex >= 0 ? overIndex + modifier : overList.length,
        0,
        activeItem,
      );
  ...
  const renderTree = (items, parentPath, parentItem = null) => {
    const parentId =
      parentPath.length > 0 ? parentPath[parentPath.length - 1].id : "root";
        ...
        >
          <EmptyDropZone
            ...
            onEditContent={
              parentItem
                ? () =>
                    setEditingContent({ item: parentItem, pathArr: parentPath })
                : null
            }
          />
        </SortableContext>
    ...
    return (
      <div className={parentPath.length > 0 ? "p-2 sm:p-4" : "space-y-6"}>
        ...
        >
          <div
            className={
              parentPath.length === 0 ? "space-y-6" : "space-y-3 min-h-[40px]"
            }
          >
            {items.map((item) => {
              const currentPath = [
      ...
      <div className="bg-surface/50 backdrop-blur-md border border-border/60 rounded-3xl p-4 sm:p-6 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-5 shadow-sm">
        <div>
          ...
          <p className="text-sm text-text-muted font-medium mt-2 leading-relaxed max-w-xl">
            Visually manage your curriculum. Nest modules, chapters, or topics
            freely inside each other, drag to reorder, and edit content
            dynamically.
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <Button
            variant="ghost"
            onClick={expandAll}
            className="hidden md:flex border border-transparent hover:border-border shadow-sm"
          >
            Expand All
          </Button>
          <Button
          ...
          <Button
            ...
            className="shadow-lg shadow-primary/20 w-full sm:w-auto py-2.5"
          >
        ...
        >
          {subjects.length === 0 ? (
            <div className="py-24 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-base text-text-muted mt-2 max-w-md font-medium leading-relaxed">
                Start building your flexible course structure by adding your
                first item.
              </p>
              <Button
                onClick={() =>
            ...
            <div className="h-16 sm:h-20 border-b border-border flex items-center justify-between px-4 sm:px-8 shrink-0 bg-surface/90 backdrop-blur-xl shadow-sm">
              <div className="flex items-center gap-3 sm:gap-4">
                <button
                  ...
                  className="p-2 sm:p-2.5 rounded-xl border border-border bg-surface hover:bg-surface-alt hover:border-primary/30 text-text-muted hover:text-primary transition-all group shadow-sm cursor-pointer"
                >
                  <ArrowLeft
                    size={20}
                ...
                <div>
                  <h3 className="font-bold text-text-primary text-sm sm:text-lg flex items-center gap-2">
                    Editing{" "}
                    <span className="text-primary truncate max-w-[150px] sm:max-w-xl">
                      {editingContent.item.title}
                    </span>
                  </h3>
                  <p className="text-[10px] sm:text-xs text-text-muted font-medium hidden sm:block">
                    Changes are auto-saved in your current session.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <Button
                  onClick={() => setEditingContent(null)}
                  className="shadow-lg shadow-primary/20 px-4 sm:px-6"
                >
                  <Save size={18} className="mr-2 hidden sm:block" /> Done
                  Editing
                </Button>
              </div>
            </div>
          ...
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                const type =
                  showItemModal.mode === "add" && typeSelect
                    ? typeSelect.value
                    : null;
          ...
          {showItemModal.mode === "add" && (
            <div>
              ...
              >
                {(() => {
                  const parentType =
                    showItemModal.parentPath.length > 0
                      ? showItemModal.parentPath[
                          showItemModal.parentPath.length - 1
                        ].type
                      : null;
                  return (
                    <>
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-3xl bg-red-500/10 border border-red-500/20 flex flex-col items-center text-center">
            ...
            <p className="text-sm text-text-muted mt-2 leading-relaxed font-medium">
              ...
              <strong className="text-red-500">{deleteConfirm.title}</strong>.
              This action will permanently remove it and all of its nested
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              variant="danger"
              className="!bg-red-500 shadow-lg shadow-red-500/20"
...
function CurriculumNode({
  ...
  const getTypeConfig = (t) => {
    switch (t?.toLowerCase()) {
      case "subject":
        return { icon: Book, colorClass: "bg-primary/10 text-primary" };
      case "module":
        return {
          icon: Layers,
      ...
      default:
        return { icon: File, colorClass: "bg-blue-500/10 text-blue-500" };
    }
  };
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`rounded-2xl border ${isTopLevel ? "shadow-md border-border/80" : "shadow-sm border-border/50"} ${isDragging ? "border-primary shadow-2xl ring-4 ring-primary/20 bg-surface" : "bg-surface hover:border-border transition-colors"} overflow-hidden relative`}
    >
      <div
        className={`flex items-center justify-between p-3 sm:p-4 group ${expanded && hasChildren ? "border-b border-border/40 bg-surface-alt/30" : "hover:bg-surface-alt/10"} transition-colors gap-2 sm:gap-4`}
      >
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
          <button
            {...attributes}
            {...listeners}
            className="p-1.5 sm:p-2.5 text-text-muted/60 hover:text-primary cursor-grab active:cursor-grabbing hover:bg-surface-alt rounded-xl transition-colors shrink-0"
          ...
          <div
            className={`p-2.5 rounded-xl ${colorClass} shrink-0 shadow-inner hidden xs:flex`}
          >
            <Icon size={20} />
          </div>

          <span className="font-bold text-text-primary text-sm sm:text-lg truncate select-none leading-tight">
            {item.title}
          </span>
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 shrink-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          {onAdd && (
            <Button
              ...
              size="sm"
              onClick={onAdd}
              className="h-9 px-2 sm:px-3 text-xs border border-transparent hover:border-border hidden md:flex"
            >
              <Plus size={14} className="md:mr-1.5" />{" "}
              <span className="hidden xl:inline">Add Inside</span>
            </Button>
          )}
          {onEditContent && (
            <Button
              ...
              className={`h-9 px-2 sm:px-3 text-xs border border-transparent shadow-sm ${hasContent ? "text-emerald-500 hover:text-emerald-600 hover:bg-emerald-500/10 hover:border-emerald-500/20" : "text-primary hover:text-primary hover:bg-primary/10 hover:border-primary/20"}`}
            >
              <FileEdit size={14} className="sm:mr-1.5" />{" "}
          ...
          <button
            onClick={onDelete}
            className="p-2 sm:p-2.5 text-text-muted hover:text-red-500 rounded-xl hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all cursor-pointer shadow-sm"
            title="Delete"
          >
            <Trash2 size={16} />
          </button>
          ...
          {onAdd && (
            <button
              onClick={onAdd}
              className="p-2 text-text-muted hover:text-primary rounded-xl hover:bg-surface-alt transition-colors md:hidden border border-transparent hover:border-border cursor-pointer shadow-sm ml-1"
              title={`Add Item`}
            >
              <Plus size={18} />
            </button>
...
function EmptyDropZone({ id, onAdd, onEditContent }) {
  const { setNodeRef, isOver } = useDroppable({ id });

  return (
    <div
      ref={setNodeRef}
      className={`p-4 sm:p-6 text-center border-2 border-dashed rounded-2xl m-2 sm:m-4 shadow-sm min-h-[120px] flex flex-col justify-center items-center transition-colors ${isOver ? "border-primary bg-primary/10" : "border-border/60 bg-surface/30"}`}
    >
      <p className="text-sm text-text-primary font-bold mb-4">
        {isOver
          ? "Drop items here to nest them"
      ...
      <div className="flex flex-wrap justify-center gap-3">
        <Button
          ...
          variant="ghost"
          onClick={onAdd}
          className="border border-border/50 bg-surface shadow-sm hover:border-primary/50 pointer-events-auto"
        >
        ...
        {onEditContent && (
          <Button
            ...
            variant="ghost"
            onClick={onEditContent}
            className="border border-border/50 bg-surface shadow-sm hover:border-primary/50 pointer-events-auto text-primary"
          >
            <FileEdit size={16} className="mr-2" /> Add Content
          </Button>
        )}
      </div>
    </div>
